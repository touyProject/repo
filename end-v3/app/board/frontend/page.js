...
const page = () => {
    ...
    const fetchList = () => {
        fetch(`/api/v1/list/frontend/all`)
            .then(r => r.json())
            .then((result) => {
    ...
    return (
        ...
        <div className='flex flex-col m-4 items-center justify-center h-screen'>
            <div><p className='text-6xl text-gray-900 mb-4'>프론트엔드 전체게시판</p></div>
            {/*서치 박스*/}
            ...
            <div className="flex mt-4 justify-between text-sm text-gray-800">
                <div>
                    <Link href="/board/frontend">전체</Link>
                    <div id='inlineBlock' className="inline-block mx-3 h-3 w-px bg-gray-400 align-middle" />
                    <Link href="/board/frontend/free">자유</Link>
                    <div id='inlineBlock' className="inline-block mx-3 h-3 w-px bg-gray-400 align-middle" />
                    <Link href="/board/frontend/question">질문</Link>
                    <div id='inlineBlock' className="inline-block mx-3 h-3 w-px bg-gray-400 align-middle" />
